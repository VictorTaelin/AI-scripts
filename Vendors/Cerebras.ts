...
/**
 * Cerebras wrapper using OpenAI compatibility.
 * Supports qwen-3-32b reasoning model.
 */

type Role = "user" | "assistant" | "system";

export class CerebrasChat implements ChatInstance {
  ...
  async ask(userMessage: string | null, options: AskOptions = {}): Promise<string | { messages: any[] }> {
    ...
    try {
      ...
      console.error("[CerebrasChat] API error:", err?.message || err);
      throw err;
    }
